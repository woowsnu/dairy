...
- 감정 카테고리, 감정 단어 간 값을 주고 받는 부분 실패❌<br/>
1차시도) 처음 구조는 감정 카테고리, 감정 단어 컴포넌트가 부모-자식 관계가 아니였고, 둘 사이에 부모 컴포넌트라고 할만한 것도 없어서 형제 컴포넌트라고 하기에도 애매했다. 그래서 두 컴포넌트 간 props 전달을 Link의 query를 사용해 JSON형태로 전달했다.
전달에는 성공했으...props를 받은 감정 단어 컴포넌트에서 넘겨받은 JSON형식 데이터 parse를 한 후 받아온 데이터가 배열 타입인 것까지 확인을 했는데 계속 객체로 인식되어 map 함수 사용이 되지 않았다. <br/>
2차시도) 중앙에서 상태 값을 관리하고 액션을 등록할 수 있는 리덕스를 학습하고 진행할까하여 잠시 영상을 봤으나 도저히 기간내에 되지 않을 것 같아 후퇴<br/>
3차시도) 1차의 방법으로 계속 하던 중 Ref관련 오류가 발생해 강사님이 알려주신 useRef, FowardRef 학습 후 적용해 보았으나 제대로 적용되지 않았다. 2,3차의 내용들은 꼭 학습해야겠다고 느낌<br/>
4차시도) 결국 props를 넘겨주기 쉬운 부모-자식 형태로 컴포넌트 위계를 변경했다. 부모컴포넌트에서 배열 타입으로 초기화한 state에 자식 컴포넌트에서 클릭이벤트로 넣어주고자 하였으나, 계속 하나의 값만 들어가게 되어 결국 단어 선택을 하나만 하는 걸로 마무리하였다.<br/>
